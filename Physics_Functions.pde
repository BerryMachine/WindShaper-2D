...
float findDivergence (Fluid f, int x, int y) {
  return (f.u[IX(x+1, y)] - f.u[IX(x, y)]) +
  ...
  for (int iter = 0; iter < iters; iter++) {
    ...
    for (int i = 1; i < f.numX-1; i++) {
      for (int j = 1; j < f.numY-1; j++) {
        ...
        // Only update cells where (i + j) % 2 == 0  (the "red" cells)
        if ((i + j) % 2 == 0) {
          
          // Skip if this cell or all neighbors are solid
          float sAdj = f.s[IX(i-1, j)] +
                       f.s[IX(i+1, j)] +
                       f.s[IX(i, j-1)] +
                       f.s[IX(i, j+1)];
    ...
    for (int i = 1; i < f.numX-1; i++) {
      for (int j = 1; j < f.numY-1; j++) {
        ...
        // Only update cells where (i + j) % 2 == 1 (the "black" cells)
        if ((i + j) % 2 == 1) {
...
float interpolate (Fluid f, float[] arr, float x, float y, float offsetX, float offsetY) {
  float gridX = (x - offsetX) / float(f.size);
  int iLeft = floor(gridX);
  int iRight = min(iLeft + 1, f.numX - 1);
  float xFactor = gridX - iLeft;
