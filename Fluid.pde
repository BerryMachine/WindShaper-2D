...
class Fluid {
  ...
  void drawFluid () {
    ...
    // Find min/max pressure for coloring
    float minP = p[0];
    float maxP = p[0];
    for (int i = 0; i < numCells; i++) {
      ...
      for (int j = 0; j < numY; j++) {
        ...
        if (scene.show_pressure) {
          ...
          if (s[IX(i, j)] == 0.0) {
            ...
            // Grayscale smoke from 0..1 => 0..255
